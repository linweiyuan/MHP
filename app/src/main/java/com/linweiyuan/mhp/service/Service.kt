...
object Service {
    ...
    private fun <T> create(clazz: Class<T>) = Proxy.newProxyInstance(clazz.classLoader, arrayOf<Class<*>>(clazz)) { _, method, args ->
        // @Api注解
        ...
        ctx.doAsync {
            try {
                val token = ctx.getSharedPreferences(Constant.SP_FILE_NAME, AppCompatActivity.MODE_PRIVATE).getString(Constant.SP_TOKEN, null)
                    ...
                    when (e) {
                        is HttpStatusException -> {
                            if (e.statusCode == 401) {
                                ...
                                ctx.getSharedPreferences(Constant.SP_FILE_NAME, AppCompatActivity.MODE_PRIVATE)
                                    .edit()
                                    .clear()
                                    .apply()
